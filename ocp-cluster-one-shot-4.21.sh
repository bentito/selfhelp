...
if [ -z "$(ls -A "$CRED_REQ_DIR" 2>/dev/null)" ]; then
    ...
    # We MUST pass the public key generated by `openshift-install create manifests` to ccoctl,
    # otherwise ccoctl generates its own keys, creating a crypto mismatch with the cluster.
    podman run --rm \
        ...
        bash -c "apt-get update -qq && apt-get install -y -qq ca-certificates && /bin-mount/ccoctl aws create-all --name=\"$OCP_CLUSTER_NAME\" --region=\"$OCP_REGION\" --credentials-requests-dir=\"/data/credrequests\" --public-key-file=\"/data/tls/bound-service-account-signing-key.pub\" --output-dir=\"/data/ccoctl-output\"" || die "ccoctl provisioning failed via podman"
